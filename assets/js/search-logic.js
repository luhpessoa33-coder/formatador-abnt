...
async function buscarCientificoV3() {
    ...
    try {
        ...
        data.message.items.forEach(item => {
            ...
            div.innerHTML = `
                <strong>${titulo.substring(0, 60)}...</strong><br>
                <span class="text-info">${family} (${ano})</span>
                <button class="btn btn-xxs btn-primary mt-1 w-100" onclick="registrarCitar('${family}', '${ano}', '${titulo}')">+ Citar</button>
            `;
...
function baixarBiblioTxt() {
    const blob = new Blob([biblioGlobal.sort().join('\n\n')], { type: 'text/plain' });
